...
def _build_tasks(
    ...
    if args.start_url or args.output_dir:
        start_url_value = core._select_task_value(args.start_url, None, config, "start_url")
        start_url_str = str(start_url_value) if start_url_value is not None else ""
        output_dir_value = core._select_task_value(args.output_dir, None, config, "output_dir")
        parser_spec = core._select_task_value(None, None, config, "parser")
        if args.verify_local:
            verify_local = True
        else:
            verify_local = bool(config.get("verify_local", False))
        state_file = core._select_task_value(args.state_file, None, config, "state_file")
        structure_file = core._select_task_value(None, None, config, "structure_file")
    ...
    if isinstance(tasks_config, list) and tasks_config:
        for index, raw_task in enumerate(tasks_config):
            ...
            if args.task and args.task != name:
                continue
            start_url_value = core._select_task_value(None, raw_task, config, "start_url")
            start_url_str = str(start_url_value) if start_url_value is not None else ""
            output_dir = core._select_task_value(None, raw_task, config, "output_dir")
            parser_spec = core._select_task_value(None, raw_task, config, "parser")
            ...
            if args.verify_local:
                ...
                verify_local = bool(config.get("verify_local", False))
            state_file = core._select_task_value(None, raw_task, config, "state_file")
            structure_file = core._select_task_value(None, raw_task, config, "structure_file")
            task_specs.append(
                TaskSpec(
                    name=name,
                    start_url=start_url_str,
...
def _prepare_task_layout(
    ...
    if task.from_task_list:
        pages_dir = os.path.join(pages_base, core.safe_filename(task.name))
    else:
        pages_dir = pages_base

    output_value = task.output_dir if task.output_dir else None
    if output_value:
        output_dir = core._normalize_output_path(
            output_value,
            artifact_dir,
            "downloads",
        ...
        if task.from_task_list:
            default_segment = core.safe_filename(task.name) or "downloads"
    ...
    state_value = core._select_task_value(
        state_pref,
        task.raw_config,
        config,
        "state_file",
    ...
    state_file = core._resolve_artifact_path(
    ...
    build_value = core._select_task_value(
    ...
    download_value = core._select_task_value(
    ...
    preview_value = core._select_task_value(
        args.preview_page,
        task.raw_config,
        config,
...
def _prepare_http_options(
    ...
    delay = float(core._select_task_value(args.delay, task.raw_config, config, "delay", 3.0))
    jitter = float(core._select_task_value(args.jitter, task.raw_config, config, "jitter", 2.0))
    timeout = float(core._select_task_value(args.timeout, task.raw_config, config, "timeout", 30.0))
    min_hours = float(core._select_task_value(args.min_hours, task.raw_config, config, "min_hours", 20.0))
    max_hours = float(core._select_task_value(args.max_hours, task.raw_config, config, "max_hours", 32.0))
    return HttpOptions(
        delay=delay,
        jitter=jitter,
        timeout=timeout,
...
def _prepare_cache_behavior(
    ...
    if not prefetch_requested:
        config_cache_listing = core._select_task_value(
            None,
            task.raw_config,
            config,
            "cache_listing",
